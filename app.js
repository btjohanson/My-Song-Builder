(() => {
  ...
  const ENHARMONIC_DISPLAY = {1:"C#",3:"Eb",6:"F#",8:"Ab",10:"Bb"};
  ...
  const tuningPresets = [
    ...
    {id:"guitar-eb-standard-6",label:"Eb Standard — Eb Ab Db Gb Bb Eb",strings:6,notes:["Eb2","Ab2","Db3","Gb3","Bb3","Eb4"]},
  ...
  function intervalSetForRoot(pcs,root){return [...new Set(pcs.map(pc=>(pc-root+12)%12))].sort((a,b)=>a-b);}
  function arraysEqual(a,b){return a.length===b.length&&a.every((v,i)=>v===b[i]);}
  function libraryKey(){return `${state.stringCount}|${state.tuning.join(",")}`;}

  function getLibrary(){
    try{return JSON.parse(localStorage.getItem("msb-chord-library")||"{}");}
    catch{return {};}
  }
  function setLibrary(lib){localStorage.setItem("msb-chord-library",JSON.stringify(lib));}
  ...
  function identifyChord(frets){
    ...
    pcs.forEach(root=>{
      ...
      chordTemplates.forEach((tpl,idx)=>{
        if(arraysEqual(ints,tpl.intervals)){
          let score=100-idx*.02;
          if(root===bassPc)score+=4;
          candidates.push({root,tpl,score});
          return;
        }
  ...
  function makeChord(){return {id:uid("ch"),frets:Array(state.stringCount).fill("X"),note:""};}
  function makeSection(){
    return {id:uid("sec"),letter:nextSectionLetter(state.sections.length),name:"",note:"",chords:[makeChord()]};
  }
  ...
  function populateTuningPresets(){
    ...
    fits.forEach(p=>{
      ...
      opt.value=p.id;opt.textContent=p.label;select.appendChild(opt);
    });
    select.value=fits.some(p=>p.id===state.tuningPreset)?state.tuningPreset:"custom";
  }
  ...
  function renderTuningEditors(){
    ...
    state.tuning.forEach((n,i)=>{
      ...
      label.innerHTML=`S${i+1}<input data-tuning-index="${i}" value="${escapeAttr(n)}" aria-label="String ${i+1} tuning">`;
  ...
  function renderSections(){
    ...
    state.sections.forEach(sec=>{
      ...
      node.querySelector(".add-to-song-btn").addEventListener("click",()=>{
        addSectionToArrangement(sec.id);
        renderArrangement();renderOutputs();
      });
      node.querySelector(".add-chord-btn").addEventListener("click",()=>{
        sec.chords.push(makeChord());renderSections();renderOutputs();
      });
  ...
  function renderChord(sec,ch,chIndex){
    ...
    ch.frets.forEach((f,i)=>{
      ...
      inp.addEventListener("change",()=>{
        ...
        updateChordDetection(row,ch);renderOutputs();renderLibrarySummary();
      });
    ...
    row.querySelector(".duplicate-chord-btn").addEventListener("click",()=>{
      ...
      clone.id=uid("ch");
      sec.chords.splice(idx+1,0,clone);
  ...
  function reletterSections(){state.sections.forEach((s,i)=>s.letter=nextSectionLetter(i));}
  ...
  function renderArrangement(){
    ...
    state.arrangement.forEach((item,index)=>{
      ...
      row.innerHTML=`
        ...
        <div class="arrangement-mobile-actions">
          <button class="move-up secondary" title="Move up" ${index===0?"disabled":""}>↑</button>
          <button class="move-down secondary" title="Move down" ${index===state.arrangement.length-1?"disabled":""}>↓</button>
          <button class="copy-arrangement secondary" title="Duplicate">Copy</button>
      ...
      row.querySelector(".move-down").addEventListener("click",()=>{
        if(index>=state.arrangement.length-1)return;
        [state.arrangement[index+1],state.arrangement[index]]=[state.arrangement[index],state.arrangement[index+1]];
        renderArrangement();renderOutputs();
      });
      row.querySelector(".copy-arrangement").addEventListener("click",()=>{
        const clone=JSON.parse(JSON.stringify(item));
        clone.id=uid("arr");
        state.arrangement.splice(index+1,0,clone);
        renderArrangement();renderOutputs();
      });
      row.querySelector(".remove-arrangement").addEventListener("click",()=>{
        state.arrangement=state.arrangement.filter(a=>a.id!==item.id);renderArrangement();renderOutputs();
      });
  ...
  function addSectionToArrangement(sectionId){
    state.arrangement.push({id:uid("arr"),sectionId,repeats:1,note:""});
  }
      ...
      if(dragPayload.type==="section"){
        ...
        if(copy){
          ...
          if(src){
            ...
            clone.id=uid("arr");
            state.arrangement.splice(idx+1,0,clone);
          }
        }
      }
      dragPayload=null;renderArrangement();renderOutputs();
    });
  }
  ...
  function keyCandidates(){
    ...
    chordEvents.forEach((ev,idx)=>{
      ...
      if(idx===0&&root!==null)totalPCWeight[root]+=.8;
  ...
  function keyName(c){return `${midiToName(c.tonic)} ${c.mode==="major"?"major":"minor"}`;}
  ...
  function renderAllComputed(){renderSections();renderArrangement();renderOutputs();renderLibrarySummary();}
  function renderAll(){
    syncTopControls();populateTuningPresets();renderTuningEditors();renderAllComputed();
  }
  ...
  function resizeStringCount(newCount){
    ...
    if(preset){
      state.tuningPreset=preset.id;state.tuning=[...preset.notes];
    }else{
      const next=[...state.tuning];
      while(next.length<newCount)next.push(next[next.length-1]||"E3");
      state.tuning=next.slice(0,newCount);state.tuningPreset="custom";
    }
  ...
  function saveSong(){
    ...
    songs[key]={...snapshot,savedAt:new Date().toISOString()};
    localStorage.setItem("msb-songs",JSON.stringify(songs));
    flashButton($("#saveSongBtn"),"Saved");
  }

  function getSavedSongs(){
    try{return JSON.parse(localStorage.getItem("msb-songs")||"{}");}
    catch{return {};}
  }
  ...
  function showLoadDialog(){
    ...
    entries.forEach(([id,s])=>{
      ...
      row.querySelector(".load-one").addEventListener("click",()=>{
        ...
        Object.assign(state,JSON.parse(JSON.stringify(s)),{_savedId:id});
        $("#loadDialog").close();renderAll();
      });
      row.querySelector(".delete-one").addEventListener("click",()=>{
        delete songs[id];localStorage.setItem("msb-songs",JSON.stringify(songs));showLoadDialog();
      });
  ...
  function newSong(){
    ...
    state.sections.push(makeSection());renderAll();
  }
  ...
  function exportPracticePNG(){
    ...
    const scale=2,width=1080,margin=68,lineH=40,headingH=56;
    const lines=[];
    ...
    state.arrangement.forEach(item=>{
      ...
      sec.chords.forEach(ch=>{
        lines.push({
          type:"chord",
          name:state.practiceDisplayMode==="both"?chordNameFor(ch):"",
          shape:ch.frets.join("  "),
          note:ch.note||""
        });
      });
    ...
    lines.forEach(l=>{
      ...
      if(l.type==="chord"){
        ...
        if(l.note){
          ctx.font="italic 17px system-ui";ctx.fillStyle="#666";ctx.textAlign="right";
          ctx.fillText(l.note,width-margin,y+25);ctx.textAlign="left";ctx.fillStyle="#111";
        }
  ...
  function wireControls(){
    ...
    $("#sectionLabelStyle").addEventListener("change",e=>{
      state.sectionLabelStyle=e.target.value;renderSections();renderArrangement();renderOutputs();
    });
    ...
    $("#clearLibraryBtn").addEventListener("click",()=>{
      ...
      localStorage.removeItem("msb-chord-library");seedLibrary();renderAllComputed();
    });
